...
def get_rate_limit_key():
    """Per-user rate limiting for logged-in users, IP-based for anonymous."""
...
def create_app():
    ...
    app.config['SQLALCHEMY_DATABASE_URI'] = os.environ.get('DATABASE_URL', 'sqlite:////var/www/citify_ynh/citify.db')
    ...
    csp = {
        'default-src': ["'self'"],
        'script-src':  ["'self'", 'cdn.jsdelivr.net', 'unpkg.com', 'rustdesk.citify.ca'],
        'style-src':   ["'self'", "'unsafe-inline'", 'cdn.jsdelivr.net',
                        'fonts.googleapis.com', 'unpkg.com'],
        'font-src':    ["'self'", 'fonts.gstatic.com', 'fonts.googleapis.com'],
        'img-src':     ["'self'", 'data:', '*.backblazeb2.com',
                        '*.tile.openstreetmap.org', 'unpkg.com',
                        'cdn.shopify.com',   # Shopify merchant images
                        '*.shopify.com',
                        '*.myshopify.com',
                        'https:',            # allow any HTTPS image (imported products)
                        ],
        'connect-src': ["'self'", '*.tile.openstreetmap.org', 'rustdesk.citify.ca'],
        'frame-src':   ["'none'"],
        'object-src':  ["'none'"],
    }
    ...
    def inject_globals():
        ...
        try:
            ...
            if cats is None:
                ...
                cache.set('all_categories', cats, timeout=3600)
        except Exception:
            cats = []
